...
; NAME:deg_pol 
;
;PURPOSE: Aaron's version of new_deg_pol.pro.   To perform polarisation analysis of three orthogonal component time
;         series data.
;
...
;
;-
pro aaron_chaston_polarization,quanx,quany,quanz,start,totpoints,rotatefield,pol_lmt,pow_lmt,  $
...
gammav=Make_Array(nosteps,nopfft/2,/double)
...
helic=Make_Array(nosteps,nopfft/2,3)
...
;if (idl_type([quanx,quany,quanz]) eq 'string') then begin  
   ...
   get_data,quanx,data=xmag   
   get_data,quany,data=ymag
   get_data,quanz,data=zmag
   totpoints = n_elements(xmag.x)
...
for k=0, nopfft/2-1 do begin
    for xyz=0,2 do begin
        ...
        if (upper(j,k) GT 0.00) then gammav(j,k)=ATAN(upper(j,k),lower(j,k)) $
        else gammav(j,k)=!DPI+(!DPI+ATAN(upper(j,k),lower(j,k)))

        lambdau(j,k,xyz,*)=exp(Complex(0,-0.5*gammav(j,k)))*lambdau(j,k,xyz,*)
        ...
        helic(j,k,xyz)=1/(SQRT(Double(lambdau(j,k,xyz,0))^2+Double(lambdau(j,k,xyz,1))^2+Double(lambdau(j,k,xyz,2))^2)/SQRT(Imaginary(lambdau(j,k,xyz,0))^2+Imaginary(lambdau(j,k,xyz,1))^2+Imaginary(lambdau(j,k,xyz,2))^2))
